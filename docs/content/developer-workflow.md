...
```
fission env create --spec --name python --image fission/python-env:0.7.0 --builder fission/python-build-env:0.7.0
```
...
```
fission function create --name calc-form --env python --source form.py
```

```
fission function create --name calc-eval --env python --source calc.py
```
...
This waits for the `pip install` to finish.  The build script runs in
the builder, on the cluster itself.

## Usage Reference

[doc all the subcommands and flags here]
...
Kubernetes has a cool extension point: Custom Resources.  These are
resources that can be set up by users
