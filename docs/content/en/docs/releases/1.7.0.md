...
The executor used to be a stateful application that stores all kubernetes resources information it manages in memory. 
Store in memory means we are not able to retrieve those information again if executor pod gets deleted/restarted. So 
...
In 1.7.0, an experiment feature allows the executor to adopt existing orphaned kubernetes objects has been added
to the codebase. When this adoption feature enabled, whenever a new executor starts it will first try to adopt all 
resources created by the old executor and all adopted resources will not be deleted like before. 

There are no silver bullets, this adoption feature will increase the total startup time of executor if there are
huge amounts of resources exist in the cluster. Also, requests for functions that have no function pods exists will fail 
if the executor is still in boot up process. The executor will wait at most 30 seconds for resources adoption and start API service.
